...
- 30 分钟预算包含 GPT 分析、权重校验、推理、烘焙和复查；资源排队、首次安装和模型下载不计入。完整权重校验采用同一执行预算，超时不启动推理。
- 针对当前 16 GiB Mac，工作台四视角和局部精修采用 12 GiB 进程组占用停止阈值；独立探针默认 11 GiB，可用 `--memory-limit-gib 12` 复现已通过的纹理试验。该保护不等于整机不会发生内存压力，实测存在交换内存活动。
...
最终质检只发现可定位的局部表面问题时，可提出一个正/左/右/背视角矩形；选区须占整幅图的 0.2% 至 35%，整体重绘、无效区域以及同时存在形体问题的结果不会走自动表面纠错。最多运行一轮本机精修，继续消耗原任务剩余预算。使用保存的原始相机（含镜头位移），修改后再渲染四面，与原图及修正前四面同时比较。只有整体可接受且没有关键特征退化时选择新候选；原候选文件始终保留。

`automatic_refine_probe.ts` 以明确标注的受控坐垫选区验证真实推理和 GPT 比较，不代表 GPT 在真实生成任务中自主选区的分支已经验收。`texture_audit_probe.ts` 用真实 Blender 检查等比缩放、明显拉伸和材质丢失。形体编辑后的 `texture-audit.json` 测量 UV 到表面的各向异性变化；数值检查不能替代照片视觉验收。

准备主体时可另上传最多五张辅助图，主图保持明确。辅助图用于 GPT 核对结构、补充纹理描述，并随版本保留 ID；不作为 Hunyuan 原生多视图输入，也不进行未验证的直接辅助视图投射。

通过文件验证但未上色的灰模也可以明确采用为可编辑版本，版本记录保留 `texture`、`visual-quality` 缺失项。取消任务不提供自动采用，不覆盖原作品。
