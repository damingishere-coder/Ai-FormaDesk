...
export function VideoRecorder({
  ...
  async function start() {
    ...
    setDuration(0);
    try {
      if (!viewport.current) throw new Error("三维画面还在加载，请稍后再录制");
      ...
      if (mime) {
        ...
        r.onstop = async () => {
          let result = new Blob(chunks, { type: r.mimeType });
          if (r.mimeType.includes("webm")) {
            try {
              result = await fixWebmDuration(
                result,
                (samples.current.at(-1)?.time || 0) * 1000,
                { logger: false },
              );
            } catch {
              setError("视频时长整理失败，已保留原始片段供下载或重新录制");
            }
          }
          blob.current = result;
    ...
    <>
      ...
      <div className="recording-bar glass" role="region" aria-label="视频录制">
        <div>
          ...
          <small>
            {settings.width} × {settings.height} · 30 fps ·{" "}
