...
const key = "forma-chat-layout-v3";
function defaults(): Layout {
  const width = Math.min(720, innerWidth - 24),
    height = Math.min(innerHeight * 0.75, innerHeight - 100);
  return {
    x: (innerWidth - width) / 2,
    y: Math.max(76, innerHeight - height - 64),
    width,
    height,
    moved: false,
  };
...
function clamp(v: Layout): Layout {
  if (!v || ![v.x, v.y, v.width, v.height].every(Number.isFinite))
    v = defaults();
...
export function useChatLayout(expanded: boolean) {
  ...
  const [collapsedHeight, setCollapsedHeight] = useState(210);
