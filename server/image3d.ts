...
export async function imageEnvironment() {
  try {
    const result = await runProcess(
      imagePython(),
      [
        path.join(ROOT, "scripts/image3d/health.py"),
        "--runtime",
        IMAGE3D_RUNTIME,
      ],
      { timeout: 30000 },
...
export async function runImageProbe(
  ...
  pipeline: boolean | "refine" = false,
  shapeReview?: (attempt: number, remainingSeconds: number) => Promise<unknown>,
) {
  let last = "";
  ...
  const inspect = () => {
    try {
      ...
      if (value !== last) {
        ...
        progress(report);
        const attempt = report.shapeReviewPending;
        if (
          shapeReview &&
          [0, 1].includes(attempt) &&
          !decisions.has(attempt)
        ) {
          decisions.add(attempt);
          reviewing = shapeReview(attempt, report.shapeReviewRemainingSeconds)
            .catch((error) => ({
              action: "stop",
              error: (error as Error).message,
            }))
            .then((decision) => {
              const target = path.join(dir, `shape-decision-${attempt}.json`);
              fs.writeFileSync(target + ".tmp", JSON.stringify(decision));
              fs.renameSync(target + ".tmp", target);
            });
        }
      }
    } catch {
      /* atomic report may not exist before preflight completes */
    }
  };
  ...
  try {
    const result = await runProcess(
      ...
      [
        path.join(
          ROOT,
          pipeline === "refine"
            ? "scripts/image3d/refine.py"
            : pipeline
              ? "scripts/image3d/pipeline.py"
              : "scripts/image3d/probe.py",
        ),
        "--runtime",
        IMAGE3D_RUNTIME,
        "--job",
    ...
    if (result.code)
      throw new Error(report.error || "本地图生建模失败，已保留候选");
    return report;
  } finally {
    clearInterval(timer);
    inspect();
...
export function preparedImage(pid: string, id: string) {
  ...
  if (!value || value.projectId !== pid)
    throw new Error("处理图片不属于当前作品");
  for (const aid of [
    value.attachmentId,
    value.sourceId,
    value.imageId,
...
export async function prepareImage(
  ...
  try {
    await runImageProbe(
      dir,
      ["--case", "foreground", "--image", attachmentPath(pid, source.id)],
      signal,
      progress,
    );
    const instances = JSON.parse(
      fs.readFileSync(path.join(dir, "foreground/instances.json"), "utf8"),
    );
  ...
  return put<PreparedImage>("prepared-image", {
    id: uid(),
    projectId: pid,
    attachmentId: aid,
    sourceId: source.id,
    ...
    variants,
    createdAt: now(),
    status: warning || variants.length > 1 ? "needs-selection" : "ready",
    warning,
  });
}

export async function savePreparedMask(
  pid: string,
  id: string,
  input: Buffer,
  ...
  if (
    Object.values(region).some((n) => !Number.isInteger(n) || n < 0) ||
    region.width < 1 ||
    region.height < 1 ||
    region.left + region.width > old.width ||
    region.top + region.height > old.height
  )
    throw new Error("裁切区域越界");
  const alpha = await decoder
    .removeAlpha()
    .greyscale()
    .extract(region)
    .raw()
  ...
  const rgb = await sharp(attachmentPath(pid, old.sourceId))
    .removeAlpha()
    .extract(region)
    .png()
    .toBuffer();
  ...
  return put<PreparedImage>("prepared-image", {
    ...
    variants: [{ imageId: image.id, maskId: mask.id }],
    status: "ready",
    warning: undefined,
    createdAt: now(),
  });
