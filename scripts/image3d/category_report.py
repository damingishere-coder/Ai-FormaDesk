...
for index,sample in enumerate(report['samples']):
    ...
    articles.append(f'<article><h2>{categories[sample["category"]]} · 样本 {index+1}</h2><p class="status">实验性 · {html.escape(sample["status"])}</p><p>{html.escape(summary)}</p><div class="images">{"".join(figures)}</div><p>{credit}</p><p>输入限制：{html.escape(sample.get("inputLimitations",""))}</p><ul>{details}</ul><small>任务：{html.escape(sample.get("jobId","尚未生成"))}</small></article>')
