...
def main():
    ...
    space(job,0)
    weights=verify(runtime,{w['name'] for w in LOCK['weights'] if w['name']!='shape-small'})
    sources={}
    ...
    try:
        with r.acquired():
            ...
            r.report.update(status='backend-passed',result=json.loads((job/'refine-report.json').read_text()),executionSeconds=r.budget-r.remaining());r.save()
    except Exception as error:r.report['error']=str(error);r.save();raise
